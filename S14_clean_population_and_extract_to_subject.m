...
outPath = fullfile(Path,'Population');
mkdir(outPath);
ns = length(List);
order = 4;
...
for i = 1:length(nodes)
    ...
    for v = 1:length(List)
        filename = fullfile(Path,List{v},'Parcels_7Networks',strcat(num2str(nodes(i,2)),'_',num2str(nodes(i,3))),'track_native_rs_in_yeo_space.tck');
        outPath2 = fullfile(Path,List{v},'Parcels_7Networks',strcat(num2str(nodes(i,2)),'_',num2str(nodes(i,3))),'cluster_in_yeo_space');
        mkdir(outPath2);
